...
class Classificator:
    ...
    def predict_proba_class(self, image: Image.Image) -> List[torch.Tensor]:
        ...
        return predicted_class, probabilities[0][predicted_class].cpu().numpy()

    def predict(self, image: Image.Image) -> torch.Tensor:
        return self.predict_proba_class(image)[0]

    def predict_embedding(self, image: Image.Image) -> torch.Tensor:
        processed_image = self._transform(image).unsqueeze(0)
    ...
    def predict_result(self, image: Image.Image) -> Dict[str, torch.Tensor]:
